...
/**
 * The `?blockers=` query filter, built here rather than beside the route so the frontier's "no open
 * blocker" clause and the `_count` a client reads can never drift apart.
 *
 * This arrived twice — #21 built it for `GET /tasks?blockers=none` while #24 rebuilt the same fact
 * for the dependency routes — and the two agreed on the meaning while differing on the shape: a
 * callable taking the filter value, against a bare `where` constant. The callable wins because
 * `blockers=any` has no constant form, and an undefined filter must contribute no clause at all
 * rather than an empty one. Both now read the single `openBlockerEdgeWhere` above, so there is one
 * definition of an open blocker in the codebase and no alias pretending to be a second.
 */
export function openBlockersWhere(
  filter: 'none' | 'any' | undefined
): Prisma.TaskWhereInput['blockingDependencies'] {
  if (!filter) return undefined;
  return filter === 'none' ? { none: openBlockerEdgeWhere } : { some: openBlockerEdgeWhere };
}
